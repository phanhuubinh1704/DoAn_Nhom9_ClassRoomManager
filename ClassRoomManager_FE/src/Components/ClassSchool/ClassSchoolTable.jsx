...
import Add_Update_ClassSchool from './Form/Add_Update_ClassSchool';
import { getClassSchools, deleteClassSchool } from '../../Service/ClassSchoolService';
const { confirm } = Modal;
const ClassSchoolTable = () => {
  ...
  const loadData = async () => {
    try {
      const result = await getClassSchools();
  ...
  const handleDelete = async (item) => {
    let result = null
    result = await deleteClassSchool(item.classSchoolId)
    ...
    <>
      <Row style={{}}>
        ...
        <Col md={16} style={{ textAlign: 'right' }}>
          ...
          <Button type="primary" onClick={Create_Action}>Tạo lớp học</Button>
...
export default ClassSchoolTable;