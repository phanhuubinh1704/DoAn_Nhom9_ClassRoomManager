...
const StaffTable = () => {
  ...
  const delete_Action = async (item) => {
    confirm({
      title: 'Bạn chắc chắn muốn xóa nhân viên này không?',
  ...
  const handleDelete = async (item) => {
    let result = null
    result = await deleteStaff(item.userId)
    ...
    <>
      <Row style={{}}>
        ...
        <Col md={16} style={{ textAlign: 'right' }}>
          ...
          <Button type="primary" onClick={Create_Action}>Thêm nhân viên</Button>
